...
public class HtmlReportConfig {
	 ...
	 * Constructor.
	 *
	 * @param type            the typ of report (this is not "HTML", but rather
	 *                        "Model coverage report".)
	 * @param creator         the helper that creates the actual report
	 * @param templateFactory The template data factory.
	 */
	public HtmlReportConfig(String type, HtmlReportCreator creator) {
	...
	public File create() {
		// Sanity-check and avoid recursive invocations from extensions that try to be
		// funny.
		try {
			sanityCheck();
			createCalled = true;
